...
function buildImprovementTips(sessions: UserSessionSummary[]): string[] {
  ...
  // Most recent next_session_plan
  for (const s of sessions.slice(0, 3)) {
    const r = s.report_json as CoachingReport | null;
    if (r?.next_session_plan) {
      tips.push(r.next_session_plan);
    }
  }

  // Recent drill advice (unique titles)
...
export default async function ProfilePage() {
  ...
  const sessions = await getUserSessionSummaries(user.clerk_user_id);
  ...
  const chartPoints = scoreTrend.map((s, i) => {
    const x =
      chartPadLeft +
      (scoreTrend.length > 1 ? (i / (scoreTrend.length - 1)) * plotW : plotW / 2);
        ...
        {totalSessions === 0 ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {/* Box 1: Top Objections (struggles) */}
              <div className="bg-gray-50 rounded-2xl border border-gray-200 p-6">
